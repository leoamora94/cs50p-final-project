...
AGE_CATEGORIES = ['INF', 'YTH', 'JR', 'SR', 'UNI',
...
def main():
    ...
    kid_start_list, male_start_list, female_start_list = create_start_list(
        athletes, num_platforms)
...
def create_start_list(athletes, num_platforms):
    ...
    # create start lists
    kid_start_lists = [kid_athletes[i:i+num_platforms]
                       for i in range(0, len(kid_athletes), num_platforms)]
    male_start_lists = [male_athletes[i:i+num_platforms]
                        for i in range(0, len(male_athletes), num_platforms)]
    female_start_lists = [female_athletes[i:i+num_platforms]
                          for i in range(0, len(female_athletes), num_platforms)]
...
def generate_pdf_start_lists(start_lists, output_filename, genre):
    ...
    for index, start_list in enumerate(start_lists):
        if genre == 'K':
            data = [[f'GRUPO {genre}{index+1}  -  PESAGEM: {DAY_ONE} às {
                WEIGHT_START_TIME+index*2}:00  -  INICIO: {DAY_ONE} às {START_TIME+index*2}:00']]
        elif genre == 'F':
            data = [[f'GRUPO {genre}{index+1}  -  PESAGEM: {DAY_TWO} às {
                WEIGHT_START_TIME+index*2}:00  -  INICIO: {DAY_TWO} às {START_TIME+index*2}:00']]
        elif genre == 'M':
            data = [[f'GRUPO {genre}{index+1}  -  PESAGEM: {DAY_THREE} às {
                WEIGHT_START_TIME+index*2}:00  -  INICIO: {DAY_THREE} às {START_TIME+index*2}:00']]

        data.append(['SOBRENOME', 'NOME', 'DT NASC', 'CATEGORIA',
                    'ARRANCO', 'ARREMESSO', 'TOTAL'])
        ...
        table.setStyle(TableStyle([
            ...
            # Right-align CATEGORIA column
            ('ALIGN', (3, 0), (3, -1), 'RIGHT'),
            # Center-align the last three columns (ARRANCO, ARREMESSO, TOTAL)
            ('ALIGN', (4, 0), (-1, -1), 'CENTER'),
            # Left-align HOURS (first row)
            ('ALIGN', (0, 0), (-1, 0), 'LEFT'),
            # Center-align the HEADER (second row)
